...
$ git clone https://bitbucket.org/965/cities-fe
$ cd cities-fe
...
Project will be compiled ahead of time (AOT), and then the resulting bundle will be tree-shaken and minified. During the tree-shaking process Rollup statically analyses your code, and your dependencies, and includes the bare minimum in the bundle.
...
- `cities-fe` - In development mode, this container serves the angular app. In production mode it builds the angular app, with the build artifacts being served by the Nginx container
- `cities-fe-nginx` - This container is used only production mode. It serves the built angular app with Nginx.
...
For different environments needed to be configured to use different end points, settings, etc. files `dev.ts` or `prod.ts` in`./tools/env/` can be used. The name of the file is environment you want to use.
